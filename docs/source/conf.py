...
version = importlib.metadata.version("hdiffpatch")
...
# The included README.md starts its headings at H2.
...
# LinkCode
code_url = f"https://github.com/BrianPugh/hdiffpatch-python/blob/{git_commit}"
...
def linkcode_resolve(domain, info):
    ...
    try:
        ...
        # e.g. object is a typing.Union or compiled Cython code without source
        return None
    if file is None:
        return None
    file = Path(file).resolve().relative_to(git_repo.working_dir)
    if file.parts[0] != "hdiffpatch":
